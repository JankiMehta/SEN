...
if (isset($_SESSION['id'])) {
	...
	if (isset($_POST['submit'])) {
		if(strcmp($privilege, "admin")==0) {
			$query= mysql_query("SELECT MAX(SNo) from admin",$con);
			$row = mysql_fetch_array($query);
			$admin_no1=$row['MAX(SNo)']+1;
			
			$admin_no= "admin".$admin_no1; 
			...
			$error = array();
			if(strlen($admin_id)!=9 or is_numeric($admin_id)==0 or empty($admin_id))
				$error[] =  "Invalid ID.";
			...
			if(strlen($post)>50  or empty($post))
				$error[] = "Post too Long.Max 50 characters allowed.";

			foreach ($error as $key => $values) { 
						echo '	<li>'.$values.'</li>';
					}
			$query_verify_id = "SELECT * FROM admin WHERE admin_id='$admin_id'";
			...
			if (!mysql_num_rows($result_verify_id)) {
				...
				$insert_query1="insert into user (user_id,event_generation,password)
								values('$admin_no',1,'$passwd')";
				$insert_query="insert into admin (admin_no,admin_id,f_name,m_name,l_name,post)
								values('$admin_no','$admin_id','$f_name','$m_name','$l_name','$post')";
				if(sizeof($error)==0){				
				...
				<div id="regtr">
					<form name="form1"  method ="post" onSubmit="return validateForm()">
						<table style="margin-left:50px; width:100%;">
							...
							<tr> 
								<td>Admin ID:</td>
								<td > <input required type="text" name="admin_id" placeholder="Admin ID" id="admin_id"></td>
								<tr><td></td><td><div id="admin_id1"></div></td></tr>
								<td></td> 
								<td></td> 
								<td></td>
								<td></td> 
							...
							<tr> 
								<td>First Name</td>
								<td ><input required type="text" name="f_name" placeholder="First Name" id="f_name" pattern="[a-z A-Z]{1,20}"></td>
						   ...
						   <tr> 
								<td>Middle Name</td>
								<td ><input type="text" name="m_name" placeholder="Middle Name" id="m_name"></td>
							...
							<tr>
								<td>Last Name</td>
								<td><input required type="text" name="l_name" placeholder="Last Name" id="l_name" pattern="[a-z A-Z]{1,20}"></td>
							...
							<tr>
								<td>Post</td>
								<td><input required type="text" name="post" placeholder="Designation" id="post" pattern="[a-z A-Z]{1,50}"></td>
...
<script src="assets/js/validate_admin.js"></script>  
