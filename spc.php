...
if (isset($_SESSION['id'])) {
	...
	if (isset($_POST['submit'])) {	
		if(strcmp($privilege, "admin")==0) {
			$query= mysql_query("SELECT MAX(SNo) from spc",$con);
			$row = mysql_fetch_array($query);
			$spc_no1=$row['MAX(SNo)']+1;
			$spc_no = "spc".$spc_no1; 
			$error = array();

			$SPCmember_id=$_POST['SPCmember_id'];
			$f_name=$_POST['f_name'];
			$m_name=$_POST['m_name'];
			$l_name=$_POST['l_name'];
			$post=$_POST['post']; 
			...
			if(strlen($SPCmember_id)!=9 or is_numeric($SPCmember_id)==0 or empty($SPCmember_id))
				$error[] = "Invalid ID.";
			...
			if(strlen($post)>50 or empty($l_name))
				$error[] = "Post too Long.Max 50 characters allowed.";

			foreach ($error as $key => $values) { 
						echo '	<li>'.$values.'</li>';
					}

			$query_verify_id = "SELECT * FROM spc WHERE SPCmember_id='$SPCmember_id'";
			$result_verify_id = mysql_query($query_verify_id, $con);
			if (mysql_num_rows($result_verify_id) == 0) {
				$pw = md5($spc_no);
				$insert_query1="insert into user (user_id, event_generation, password)
								values('$spc_no', 1, '$pw')";

				$insert_query="insert into spc (SPCmember_no,SPCmember_id,f_name,m_name,l_name,post,join_date)
								values('$spc_no','$SPCmember_id','$f_name','$m_name','$l_name','$post','$join_date')";
				...
				if(sizeof($error)==0){	
					...
					if (!mysql_query($insert_query,$con))
					  ...
					  {
					  header('Location: spc.php');
					  }
				}
			} else {
				echo 'Member is already registered ';	  
			}
			mysql_close($con);

		} else {
			echo "You are not privileged to enter SPC details";
		}
	}
} else {
				...
				<div id="regtr">
					<form name="form1" method ="post" onSubmit="return validateForm()">
						<table style="margin-left:50px; width:100%;">
							...
							<tr> 
								<td>SPC ID:</td>
								<td > <input required type="text" name="SPCmember_id" pattern="[+]?[0-9]{9,9}" placeholder="SPC ID" id="SPCmember_id"></td>
								<tr><td></td><td><div id="SPCmember_id1"></div></td></tr>
								<td></td> 
								<td></td> 
								<td></td>
								<td></td> 
						   ...
						   <tr> 
							...
							<tr>
								<td>Post</td>
								<td><input required type="text" name="post" placeholder="Designation" id="post"></td>
								<tr><td></td><td><div id="post1"></div></td></tr>
								<td></td> 
								<td></td> 
								<td></td>
								<td></td>
							...
							<tr>
								<td>Joining Date</td>
								<td><input required type="date" name="join_date" placeholder="Joining Date" id="join_date"></td>
								<td></td> 
								<td></td> 
								<td></td>
								<td></td>
...
<script src="assets/js/validate_spc.js"></script>
